...
<body>
	<div class="p20">
		<form name=pageForm id="pageForm" action="<%=path%>/dataitem/add"
			method="post">
			<div class="clearfix pop_input">
				<label for="input_suzhu"><span class="red mr10">*</span>字段名称：</label>
				<input type="text" name="subitemName" id="subitemName" maxlength="20" />
			...
			<div class="clearfix pop_input">
				...
				<select name="metadata.id"  style="width: 200px">
					<c:forEach var="data" items="${data}">
						<option value="${data.id}">${data.metadataName}</option>
					</c:forEach>
				</select>
			</div>
			<div class="clearfix pop_input">
				<label for="input_suzhu">描述：</label>
				<textarea rows="10" cols="10" id="subitemDes" name="subitemDes"></textarea>
			</div>
		</form>
