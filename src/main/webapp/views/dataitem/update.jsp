...
<body>
	<div class="p20">
		<form name=pageForm id="pageForm" action="<%=path%>/dataitem/update"
			method="post">
			<div class="clearfix pop_input">
				<label for="input_suzhu"><span class="red mr10">*</span>字段名称：</label>
				<input type="text" name="subitemName" id="subitemName" value="${item.subitemName}" maxlength="20" />
				<input type="hidden" id="id" name="id" value="${item.id }">
			</div>
			<div class="clearfix pop_input">
				<label for="input_suzhu"><span class="red mr10">*</span>所属元数据：</label>
				<select name="metadata.id"  style="width: 200px">
					<c:forEach var="data" items="${list}">
						<option value="${data.id}"<c:if test="${data.id eq item.metadata.id}"></c:if> >${data.metadataName}</option>
					</c:forEach>
				</select>
			</div>
			<div class="clearfix pop_input">
				<label for="input_suzhu">描述：</label>
				<textarea rows="10" cols="10" id="subitemDes" name="subitemDes">${item.subitemDes}</textarea>
			</div>
		</form>
