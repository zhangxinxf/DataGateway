<%@ page contentType="text/html;charset=UTF-8" language="java"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
<!DOCTYPE>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>佳华云</title>
<%
	out.println("<script type='text/javascript'>");
	out.println("var rootPath = '" + request.getContextPath() + "';");
	out.println("</script>");
	String path = request.getContextPath();
%>
<link rel="stylesheet" href="<%=path%>/css/common.css" />
<link rel="stylesheet" href="<%=path%>/css/content.css" />
<link rel="stylesheet"
	href="<%=path%>/js/vendor/msgbox/Styles/msgBoxLight.css" />
</head>
<body class="content_bg">
	<div class="position">
		<a href="javascript:{}" class="position_start">当前位置：系统首页</a> <a
			href="javascript:{}" class="position_item">系统管理</a> <a
			href="javascript:{}" class="position_item">用户管理</a>
	</div>
	<div class="content_box">
		<form class="content_search clearfix" name=searchForm id="searchForm"
			action="">
			<div class="left ml10">
				<label>姓名：<input type="text" name="userName"
					value="${userName}" onkeyup="onkeyCode(window.event||event)" /></label> <label>单位名称：<input
					type="text" onkeyup="onkeyCode(window.event||event)" name="company"
					value="${company}" /></label> <label>用户过期状态： <select
					name="overdueStatus">
						<option value="">--请选择--</option>
						<option value="1"
							<c:if test="${overdueStatus eq 1 }">selected</c:if>>过期</option>
						<option value="0"
							<c:if test="${overdueStatus eq 0 }">selected</c:if>>未过期</option>
				</select>
				</label>
			</div>
			<div class="left ml10">
				<a href="javascript:{}" class="search_icon"
					onclick="searchFrom('${page.pageUrl}');">搜索</a>
			</div>
		</form>
		<div class="content_comm">
			<ul>
				<li><a href="javascript:{}" id="add_xuniji" class="add_icon"
					onclick="openDialog('<%=path%>/userinfo/toAdd','添加用户',620,440);">添加</a></li>
				<li><a href="#" class="ref_icon"
					onclick="return userinfoActionConfirm('all','<%=path%>/userinfo/update','修改用户',620,370);">修改</a></li>
				<li><a href="#" class="info_icon"
					onclick="return userinfoActionConfirm('all','<%=path%>/userinfo/view','详情信息',550,380);">详情信息</a></li>
				<li><a href="#" class="login_icon" id="distribute_id"
					onclick="return userinfoActionConfirm('all','<%=path%>/userinfo/toDistributeVM','分配虚拟机',700,430);">分配虚拟机</a>
					<span class="bind_gray_icon none" id="distribute_id_gray">分配虚拟机</span></li>
				<c:if test="${user.roleInfo eq '0'}">
					<li><a href="#" class="ref_icon" id="updatePass_id"
						onclick="return userinfoActionConfirm('all','<%=path%>/userinfo/updatePassView','修改密码',550,200);">修改当前用户密码</a>
						<span class="bind_gray_icon none" id="updatePass_id_gray">修改当前用户密码</span>
					</li>
				</c:if>
			</ul>
		</div>
		<div class="content_table">
			<form name=deleteForm id="deleteForm"
				action="<%=path%>/userinfo/delete" method="post">
				<table border="0" cellspacing="0" cellpadding="0"
					id="table_one">
					<tr>
						<th width="5%"><input type="checkbox" name="checkbox"
							class="chk" onclick="CheckAll();" /></th>
						<th width="5%">序号</th>
						<th width="15%">姓名</th>
						<th width="10%">用户名</th>
						<th width="10%">用户角色</th>
					</tr>
					<c:forEach var="userinfo" items="${list}" varStatus="status">
						<tr>
							<td><label> 
										<input type="checkbox" name="id" class="chk"
											value="${userinfo.userId}" />
							</label></td>
							<td>${(page.index-1)*page.perPage+status.index+1}</td>
							<td>${userinfo.userName}&nbsp;</td>
							<td>${userinfo.userName}&nbsp;</td>
							<td>${userinfo.userName}&nbsp;</td>
						</tr>
					</c:forEach>
				</table>
			</form>
			<div class="table_bot">
				<div class="table_bot_comm">
					<img src="<%=path%>/img/table/table_butt.gif" width="19"
						height="19" /> <a href="javascript:{}" onclick="CheckAll();">全选</a>&nbsp;&nbsp;&nbsp;&nbsp;
					<a id="delete_id" href="javascript:{}"
						onclick="return userinfoActionConfirm('delete','<%=path%>/userinfo/delete');">删除</a>
					<a id="delete_id_gray" class="table_delete_gray" href="#">删除</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<span class="p05"></span>

				</div>
				<%@ include file="../common/pagingBar.jsp"%>
			</div>
		</div>
	</div>
</body>
<script src="<%=path%>/js/jquery-1.9.1.min.js"></script>
<script src="<%=path%>/js/vendor/jquery.form/jquery.form.min.js"></script>
<script src="<%=path%>/js/vendor/lhgdialog/lhgcore.lhgdialog.min.js"></script>
<script src="<%=path%>/js/vendor/lhgdialog/openDialog.js"></script>
<script src="<%=path%>/js/vendor/msgbox/Scripts/jquery.msgBox.js"></script>
<script src="<%=path%>/js/table.js"></script>
<script src="<%=path%>/js/admin/userinfo.js"></script>
<script type="text/javascript">
$(function() {
    Table.init("table_one");
    Table.bindDblClick((function() {
        return (function(obj) {
        	openDialog("<%=path%>/userinfo/view/" + obj.id, '详情信息', 550,
						380);
			});
		}).call(this));
		Table.bindChk((function() {
			return (function(obj) {
				$("input[name='id']", obj).each(function() {
					var status = $(this).attr('status');
					var isCurr = $(this).attr('isCurr');
					var flag = $(this).attr('flag');
					if (this.checked) {
						disableOperate(status);
						disableOperateOn(isCurr, flag);
					} else {
						Check.enable("delete_id,reset_id,distribute_id");
					}
				});
				tableCheckList();
			});
		}).call(this));
	});
	var disableOperate = function(status) {
		Check.set('delete_id', (status <= 0));
	}
	var disableOperateOn = function(isCurr, flag) {
		Check.set('updatePass_id', isCurr == 1);
		Check.set('reset_id,distribute_id', (flag != 1));
	}
	function tableCheckList() {
		var _status = false;
		var _flag = false;
		$('#table_one').find('input[name="id"]').each(function() {
			var status = $(this).attr('status');
			var isCurr = $(this).attr('isCurr');
			var flag = $(this).attr('flag');
			if (this.checked) {
				if (!_status) {
					disableOperate(status);
					if (status > 0)
						_status = true;
				}
				if (!_flag) {
					disableOperateOn(isCurr, flag);
					if (flag == 1) {
						_flag = true;
					}
				}
			}
		});
	}
	/*全选操作*/
	function CheckAll() {
		Table.checkAll();
		tableCheckList();
	}

	function onkeyCode(event) {
		if (event.keyCode == 13 || event.which == 13) {
			searchFrom('${page.pageUrl}')
		}
	}
</script>
</html>
