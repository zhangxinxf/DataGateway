...
<script src="<%=path%>/js/admin/subitem.js"></script>
<script>
	$(function() {
		//加载表单验证规则
...
<body>
	<div class="p20">
		<form name=pageForm id="pageForm" action="<%=path%>/subitem/update"
			method="post">
			<div class="clearfix pop_input">
				<label for="input_suzhu"><span class="red mr10">*</span>字段名称：</label>
				<input type="text" name="fieldName" id="fieldName" maxlength="20" value="${item.fieldName}"/>
				<input type="hidden" id="id" value="${item.id}" name="id">
			</div>
			<div class="clearfix pop_input">
				<label for="input_suzhu"><span class="red mr10">*</span>字段类型：</label>
				<input type="text" name="dbType" id="dbType" maxlength="20" value="${item.dbType}"/>
			</div>
			<div class="clearfix pop_input">
				<label for="input_suzhu"><span class="red mr10">*</span>所属指标：</label>
				<select name="item.id" id="itemId" style="width: 200px">
					<c:forEach var="data" items="${list}">
						<option value="${data.id}" <c:if test="${data.id eq item.item.id}">selected</c:if>>${data.itemName}</option>
					</c:forEach>
				</select>
			</div>
			<div class="clearfix pop_input">
				<label for="input_suzhu">描述：</label>
				<textarea rows="10" cols="10" id="fieldDes" name="fieldDes">${item.fieldDes}</textarea>
			</div>
		</form>
