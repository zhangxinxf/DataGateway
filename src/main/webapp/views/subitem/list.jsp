...
<body class="content_bg">
	...
	<div class="content_box">
		<form class="content_search clearfix" name=searchForm id="searchForm"
			action="">
			<div class="left ml10">
				<label>指标项名称：<input type="text" name="fieldName"
					value="${fieldName}" onkeyup="onkeyCode(window.event||event)" /></label> <label>所属指标：
				<select name="itemId">
						<option value="">---请选择---</option>
						<c:forEach items="${data}" var="item">
							<option value="${item.id}"
								<c:if test="${item.id eq itemId }">selected</c:if>>${item.itemName}</option>
						</c:forEach>
				</select></label>
			</div>
			<div class="left ml10">
		...
		<div class="content_comm">
			<ul>
				<li><a href="javascript:{}" id="add_xuniji" class="add_icon"
					onclick="openDialog('<%=path%>/subitem/toAdd','添加指标项',520,260);">添加</a></li>
				<li><a href="#" class="ref_icon"
					onclick="return itemActionConfirm('all','<%=path%>/subitem/update','修改指标项',520,260);">修改</a></li>
			</ul>
		</div>
		<div class="content_table">
			<form name=deleteForm id="deleteForm" action="<%=path%>/subitem/delete"
				method="post">
			...
			<div class="table_bot">
				<div class="table_bot_comm">
					...
					<a id="delete_id" href="javascript:{}"
						onclick="return itemActionConfirm('delete','<%=path%>/subitem/delete');">删除</a>
