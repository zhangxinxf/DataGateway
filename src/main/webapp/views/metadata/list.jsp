		...
		<form class="content_search clearfix" name=searchForm id="searchForm" method="post"
			...
			<div class="left ml10">
				<label>名称：<input type="text" name="name"
					value="${name}" onkeyup="onkeyCode(window.event||event)" /></label>
			</div>
			<div class="left ml10">
		...
		<div class="content_comm">
			<ul>
				<li><a href="javascript:{}" id="add_xuniji" class="add_icon"
					onclick="openDialog('<%=path%>/metadata/toAdd','添加元数据',520,220);">添加</a></li>
				<li><a href="#" class="ref_icon"
					onclick="return itemActionConfirm('all','<%=path%>/metadata/update','修改元数据',520,220);">修改</a></li>
			</ul>
		</div>
		<div class="content_table">
			<form name=deleteForm id="deleteForm"
				action="<%=path%>/metadata/delete" method="post">
			...
			<div class="table_bot">
				<div class="table_bot_comm">
					...
					<a id="delete_id" href="javascript:{}"
						onclick="return itemActionConfirm('delete','<%=path%>/metadata/delete');">删除</a>
